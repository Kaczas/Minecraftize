...
namespace Minecraftize {
  public class MainWindowVM : ViewModel {
    ...
    public ICommand MinecraftizeClickCommand => new Command(MinecraftizeClick);
    ...
    public MainWindowVM() {
      ...
      this.Sizes = new List<int> { 4, 8, 16, 32 };

#if DEBUG
      string filename = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.Desktop), "zdj.png");
    ...
    public ImageSource ImageSourceFromBitmap(Bitmap bmp) {
      ...
      var imgSource = Imaging.CreateBitmapSourceFromHBitmap(handle, IntPtr.Zero, Int32Rect.Empty, BitmapSizeOptions.FromEmptyOptions());
      return imgSource;
    }
    ...
    private void UpdateImage(Bitmap bm) {
      this.ImagePanel = ImageSourceFromBitmap(bm);
    }

    private void MinecraftizeClick(object? obj) {
      _minecraftizedImage = _minecraftizer.Minecraftize(_image, Value);
